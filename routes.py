...
from terminal_app.api.response import ok, error
from terminal_app.storage.repositories import HistoryRepository
...
async def about():
    ...
    if not html_path.exists():
        return HTMLResponse("<h1>about.html not found</h1>", status_code=500)
    return HTMLResponse(html_path.read_text(encoding="utf-8"))
...
async def api_history_news(since: str = "1970-01-01T00:00:00+00:00", limit: int = 500):
    rows = await asyncio.to_thread(history_repo.news, since, min(limit, 2000))
...
async def api_history_calendar(since: str = "1970-01-01", limit: int = 400):
    rows = await asyncio.to_thread(history_repo.calendar, since, min(limit, 2000))
...
async def api_history_cb_snapshots(bank_id: str, limit: int = 80):
    rows = await asyncio.to_thread(history_repo.cb_snapshots, bank_id.lower(), min(limit, 500))
...
async def api_history_nlp(bank_id: str, limit: int = 80):
    rows = await asyncio.to_thread(history_repo.nlp, bank_id.lower(), min(limit, 500))
    return {"history": rows, "count": len(rows), "bank_id": bank_id.lower()}


# ── Domain API v2 (uniform envelope) ─────────────────────────────

@router.get("/api/news/latest")
async def api_news_latest(limit: int = 300):
    return ok(
        data=news.live_articles[:limit],
        meta={"count": min(limit, len(news.live_articles)), "last_refresh": news.last_refresh},
    )


@router.get("/api/central-banks/states")
async def api_central_banks_states():
    return ok(data=_get_cb_states_for_api(), meta={"count": len(_get_cb_states_for_api())})


@router.get("/api/market/live")
async def api_market_live():
    return ok(data=_live_market_payload())


@router.get("/api/signals/latest")
async def api_signals_latest():
    signals = generate_signals(news.live_articles, _live_market_payload())
    return ok(data=signals, meta={"count": len(signals)})


@router.get("/api/risk/snapshot")
async def api_risk_snapshot_v2():
    snap = await asyncio.to_thread(build_risk_snapshot)
    return ok(data=snap)


@router.get("/api/system/status")
async def api_system_status():
    return ok(data=await api_status())


@router.get("/api/system/health/db")
async def api_system_health_db():
    st = db_stats()
    if st.get("enabled") and not st.get("exists"):
        return error("db_missing", "SQLite file does not exist yet", {"db": st})
    if st.get("error"):
        return error("db_error", str(st.get("error")), {"db": st})
    return ok(data=st)


@router.get("/api/system/health/ibkr")
async def api_system_health_ibkr():
    s = ibkr.get_status()
    if not s.get("connected"):
        return error("ibkr_disconnected", s.get("last_error", "IBKR unavailable"), {"ibkr": s})
    return ok(data=s)


@router.get("/api/system/health/workers")
async def api_system_health_workers():
    w = workers_status_snapshot()
    unhealthy = {k: v for k, v in w.items() if v.get("error_count", 0) > 0 and not v.get("last_ok_ts")}
    if unhealthy:
        return error("workers_unhealthy", "Some workers never reported success", {"workers": w})
    return ok(data=w)
